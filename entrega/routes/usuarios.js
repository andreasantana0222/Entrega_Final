...
const secret = process.env.MY_SECRET_TOKEN; // || 'secret'
...
router.post('/usuarios/login', async (req, res) => {
  ...
  try {
    let user = await usuarios.readByUser(user => user.username === req.body.username);
...
router.put('/usuarios/actualizar/:id', async (req, res) => {
    ...
    try {
      ...
      if (id<1){
        ...
        let objeto=req.body;
        return res.type('json').send(JSON.stringify(await usuarios.update(id,objeto), null, 2) + '\n');
      }
    } catch (e) {
    ...
    router.delete('/usuarios/borrar/:id', async (req, res) => {
      ...
      try {
        ...
        if (id<1){
          ...
          return res.type('json').send(JSON.stringify(await usuarios.delete(id), null, 2) + '\n');
        }
      } catch (e) {
        console.error({error : 'producto no encontrado'})
        res.status(500).send(JSON.stringify({error : 'producto no encontrado'}));
